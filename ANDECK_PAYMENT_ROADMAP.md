...
4. **Zalo:** Gửi bill + copy sẵn 3 dòng (nút **Copy tin nhắn Zalo** — chỉ bật sau khi bấm **Đã thanh toán** trên web):
...
| `public/stylecss/deck-hub.css` | Modal quota limit, nút **Tải lên File** hub |
...
| `public/modules/deck-features/core.js` | Pill quota pool `x/y Từ`, `#adQuotaLimitOverlay` |
...
- [x] Hub quota pills: **Deck** / **Từ** viết hoa · **không** còn nút Nâng cấp cạnh upfile (entry: avatar **Nâng cấp gói** + modal giới hạn)
- [x] Nút hub **Tải lên File** (`#adUpfileBtn`) — nền đục, rõ affordance
- [x] Modal hết quota (`#adQuotaLimitOverlay`): **Nâng Cấp** + **Liên hệ Zalo** · ✕ góc · SĐT ẩn đến khi bấm Zalo · highlight **Deck**/**Từ**/**Nâng Cấp**
...
- [x] Tiêu đề bước: **Sau khi chuyển tiền xong** + 3 bước (màu: xanh **Đã thanh toán** · cam **Copy** · xanh **Mở Zalo** / SĐT)
- [x] Nút **Đã thanh toán** (xanh) → tạo đơn mock *Chờ xác minh* · ở lại modal
- [x] **Copy tin nhắn Zalo** chỉ sau **Đã thanh toán**; trước đó toast *Bạn chưa ấn thanh toán*
- [x] Template copy: `Mã đơn` · `Email:` (không chữ Andeck) · `Gói: …`
- [x] Nút **Mở Zalo gửi xác minh** → deep link
- [x] Checkbox placeholder *“Đồng ý điều khoản”* (disabled/link `#` — Legal chat 3)
...
- [x] `public/modules/10-pricing.js` — mock orders `sessionStorage` key `andeck_mock_orders` **theo email** (G1); G2 thay bằng API
- [x] Luồng: Nâng cấp → Gói → Payment → **Đã thanh toán** → đơn *Chờ xác minh* → Copy/Mở Zalo
...
| 2 | Hết quota modal → **Nâng Cấp** → cùng modal upgrade | ✅ |
| 3 | Chọn Gói 1 → payment 17k + QR | ✅ |
| 4 | Copy tin Zalo sau **Đã thanh toán** + deep link OK | ✅ |
| 5 | **Đã thanh toán** → Đơn của tôi có *Chờ xác minh* | ✅ |
| 6 | F5 → đơn còn (mock theo email, cùng browser) | ✅ |
...
#### Bổ sung UI sau review 1D (29/06/2026 — đã code, G2 phải giữ)

| Hạng mục | Quyết định UI (không đổi khi G2 trừ khi product đổi ý) |
|----------|--------------------------------------------------------|
| Hub entry nâng cấp | Avatar **Nâng cấp gói** + modal **giới hạn quota** — **không** nút Nâng cấp trên hàng quota |
| Hub import | Nút **Tải lên File**, nền đục |
| Modal giới hạn | Một câu + highlight **Deck**/**Từ**/**Nâng Cấp** · ✕ đóng · SĐT Zalo ẩn đến khi bấm **Liên hệ Zalo** |
| Payment | **Đã thanh toán** (xanh) tạo đơn · Copy Zalo **sau** thanh toán · template `Email:` không *Email Andeck* |
| Mock đơn G1 | `sessionStorage` nhóm theo email — **G2 bỏ**, dùng `GET /api/orders/mine` |
...
- [ ] Test: **Đã thanh toán** → `POST` → admin verify → quota **cộng dồn** đúng (+10/+1000 hoặc +20/+2000)
- [ ] Test: Copy Zalo **trước** thanh toán → bị chặn; **sau** → copy đúng 3 dòng
- [ ] Test: admin1 / admin2 — mỗi nick chỉ thấy đơn của mình (`GET /api/orders/mine`)
...
| 11 | Nút tạo đơn UI | **Đã thanh toán** (không *Tạo đơn chờ xác minh*) |
| 12 | Copy Zalo | Sau khi có đơn `pending`; template `Email:` (không *Email Andeck*) |
| 13 | Entry nâng cấp hub | Avatar + modal quota — **không** nút trên hàng quota |
...
- [ ] Checkbox bắt buộc trước **Đã thanh toán** / **Mua gói** (tương tự register)
...
```
Đọc D:\andeck\ANDECK_PAYMENT_ROADMAP.md §0.2 + §0.3.2 + Giai đoạn 2.
Giai đoạn 1 (1A–1D + UI polish) đã xong. Implement backend + nối UI: goi1/goi2, cộng dồn quota, admin duyệt đơn.
Giữ UX: nút Đã thanh toán → POST /api/orders, Copy Zalo sau có đơn, template Email: (không Andeck), GET /api/orders/mine.
```
